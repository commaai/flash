<script>
  ...
  // Lazy load Flash component only when needed
  let FlashComponent;
  let showFlash = false;
...
<div class="flex flex-col lg:flex-row flex-wrap">
  <main class="p-12 md:p-16 lg:p-20 xl:p-24 w-screen max-w-none lg:max-w-prose lg:w-auto h-auto lg:h-screen lg:overflow-y-auto prose dark:prose-invert prose-green bg-white dark:bg-gray-900">
    ...
    <section>
      ...
      <img src="/assets/qdl-ports.svg" alt="comma three device with two ports: lower port (1) and upper port (2)." width="450" height="300" />
  ...
  <div class="lg:flex-1 h-[700px] lg:h-screen bg-gray-100 dark:bg-gray-800">
    {#if showFlash && FlashComponent}
      <svelte:component this={FlashComponent} />
    {:else}
      <div class="flex items-center justify-center h-full">
        <button 
          on:click={loadFlash}
          class="bg-blue-600 hover:bg-blue-500 active:bg-blue-300 transition-colors text-white px-6 py-3 rounded-md"
        >
          Start Flash Tool
        </button>
      </div>
