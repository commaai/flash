// Global QDL device class
window.qdlDevice = (function() {
    class QDLDevice {
        constructor() {
            this.mode = "";
            this.cdc = new window.USBLib.usbClass();
            this.sahara = new window.SaharaProtocol.Sahara(this.cdc);
            this.firehose = new window.FirehoseProtocol.Firehose(this.cdc);
        ...
        async connectToSahara() {
            while (!this.cdc.connected) {
                ...
                if (this.cdc.connected) {
                    ...
                    let resp = await window.QDLUtils.runWithTimeout(this.sahara?.connect(), 10000);
                    if ("mode" in resp) {
                        this.mode = resp["mode"];
        ...
        async getGpt(lun, startSector=1) {
            ...
            let data = window.QDLUtils.concatUint8Array([resp.data, (await this.firehose.cmdReadBuffer(lun, startSector, 1)).data]);
            let guidGpt = new window.GPTUtils.gpt();
            const header = guidGpt.parseHeader(data, this.firehose.cfg.SECTOR_SIZE_IN_BYTES);
            if (window.QDLUtils.containsBytes("EFI PART", header.signature)) {
                ...
                data = window.QDLUtils.concatUint8Array([data, (await this.firehose.cmdReadBuffer(lun, header.partEntryStartLba, sectors)).data]);
        ...
        async getActiveSlot() {
            ...
            for (const lun of luns) {
                ...
                for (const partitionName in guidGpt.partentries) {
                    ...
                    const active = (((BigInt(partition.flags) >> (BigInt(window.GPTUtils.AB_FLAG_OFFSET) * BigInt(8))))
                                & BigInt(window.GPTUtils.AB_PARTITION_ATTR_SLOT_ACTIVE)) === BigInt(window.GPTUtils.AB_PARTITION_ATTR_SLOT_ACTIVE);
                    if (slot == "_a" && active) {
                        return "a";
            ...
            for (const lunA of luns) {
                ...
                for (const partitionNameA in guidGptA.partentries) {
                    ...
                    if (!checkGptHeader && partitionNameA.slice(0, 3) !== "xbl") { // xbl partitions aren't affected by failure of changing slot, saves time
                        gptDataA = window.GPTUtils.ensureGptHdrConsistency(gptDataA, backupGptDataA, guidGptA, backupGuidGptA);
                        if (!sameLun) {
                            gptDataB = window.GPTUtils.ensureGptHdrConsistency(gptDataB, backupGptDataB, guidGptB, backupGuidGptB);
                        }
                        checkGptHeader = true;
                    }
