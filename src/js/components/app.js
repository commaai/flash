...
export class CopyTextComponent extends Component {
...
export class AppComponent extends Component {
  constructor(container, options = {}) {
    ...
    this.copyTextComponent = null;
    this.version = import.meta.env?.VITE_PUBLIC_GIT_SHA || 'dev';
    
    // Log version
    console.info(`flash.comma.ai version: ${this.version}`);
    
    // Set version in app state
    appState.setState({ version: this.version });
  }

  renderWindowsDriverSection() {
    if (!isWindows) return '';
      ...
      <div class="flex flex-col lg:flex-row flex-wrap">
        <main class="p-12 md:p-16 lg:p-20 xl:p-24 w-screen max-w-none lg:max-w-prose lg:w-auto h-auto lg:h-screen lg:overflow-y-auto prose dark:prose-invert prose-green bg-white dark:bg-gray-900">
          ...
          <section>
            ...
            ${this.renderWindowsDriverSection()}
          </section>
          <hr />
          ...
          <div class="hidden lg:block">
            <hr />
            flash.comma.ai version: <code>${this.version}</code>
          </div>
        </main>

        <div class="lg:flex-1 h-[700px] lg:h-screen bg-gray-100 dark:bg-gray-800" id="flash-container">
        </div>
  ...
  attachEventListeners() {
    // Initialize Flash component after render
    const flashContainer = this.querySelector('#flash-container');
    if (flashContainer) {
      this.flashComponent = new FlashComponent(flashContainer);
      this.flashComponent.mount();
    }

    // Initialize Linux copy text component if needed
    if (isLinux) {
      const copyTextContainer = this.querySelector('#copy-text-container');
      if (copyTextContainer) {
        this.copyTextComponent = new CopyTextComponent(copyTextContainer, {
          text: DETACH_SCRIPT
        });
        this.copyTextComponent.mount();
      }
    }
  }
  ...
  cleanup() {
    ...
    if (this.flashComponent) {
      this.flashComponent.unmount();
      this.flashComponent = null;
    }
    
    if (this.copyTextComponent) {
      this.copyTextComponent.unmount();
      this.copyTextComponent = null;
    }
  }
}