...
export function useQdl() {
  const [step, _setStep] = createSignal(Step.INITIALIZING)
  const [message, _setMessage] = createSignal('')
  const [progress, setProgress] = createSignal(0)
  const [error, _setError] = createSignal(Error.NONE)
  ...
  const [qdl] = createSignal(new qdlDevice())

  const [manifest, setManifest] = createSignal(null)
  ...
  function setMessage(newMessage = '') {
    if (newMessage) console.info('[QDL]', newMessage)
    ...
    switch (step()) {
      ...
      case Step.DOWNLOADING: {
        ...
        async function downloadImages() {
          for await (const [image, onProgress] of withProgress(manifest(), setProgress)) {
