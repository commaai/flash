<script>
  ...
  function updateUIState() {
    // Get the UI state for the current step
    uiState = steps[step] || steps[StepCode.INITIALIZING];

    // Apply error state if there is an error
    if (error && error !== ErrorCode.NONE) {
      const unknownError = errors[ErrorCode.UNKNOWN] || {};
      const specificError = errors[error] || {};
      uiState = { ...unknownError, ...specificError, ...uiState };
    }

    // Set the title based on message, error, or status
    if (message && !error) {
      title = message + '...';
      if (progress >= 0) {
      ...
      title = uiState.status || 'Initializing...';
    }

    // Can only start if in READY state and no errors
    canStart = step === StepCode.READY && error === ErrorCode.NONE;
  ...
  function handleStart() {
    if (!managerInitialized && !initPromise) {
      // If not initialized and not already initializing, start initialization
      initPromise = initializeFlashManager().then(() => {
        // After initialization, if in READY state, start the flash process
        if (canStart) {
          qdlManager.start();
        }
      }).catch(err => {
    ...
    } else if (managerInitialized && canStart) {
      // If already initialized and in READY state, start the flash process
      qdlManager.start();
    } else if (initPromise) {
      console.log('Initialization already in progress...');
    } else {
      console.log('Cannot start, manager state:', { 
        managerInitialized, 
        canStart, 
        step, 
        error 
  ...
  async function initializeFlashManager() {
    if (managerInitialized || qdlManager) {
      console.log('Manager already initialized');
      return;
    }

    console.log('Initializing FlashManager...');
    try {
      // Step 1: Initialize the image manager
      await imageManager.init();
      
      // Step 2: Fetch the programmer
      const programmerResponse = await fetch(config.loader.url);
      if (!programmerResponse.ok) {
        throw new Error(`Failed to fetch programmer: ${programmerResponse.status}`);
      }
      const programmer = await programmerResponse.arrayBuffer();
      
      // Step 3: Create the manager with all callbacks
      qdlManager = new FlashManager(config.manifests.release, programmer, {
        ...
        onConnectionChange: (newConnected) => {
          console.log('Connection changed:', newConnected);
          connected = newConnected;
          updateUIState();
        },
        onSerialChange: (newSerial) => {
          console.log('Serial changed:', newSerial);
          serial = newSerial;
          updateUIState();
        }
      });
      ...
      console.log('FlashManager initialized successfully!');
      
      // Make sure UI state is updated
      updateUIState();
      
    } catch (err) {
      console.error('Failed to initialize FlashManager:', err);
      error = ErrorCode.UNKNOWN;
      updateUIState();
      throw err;
    }
  ...
  onMount(() => {
    ...
    initPromise = initializeFlashManager().catch(err => {
      console.error('Initialization failed on mount:', err);
      error = ErrorCode.UNKNOWN;
      updateUIState();
    }).finally(() => {
      initPromise = null;
  ...
  // Reactive statement to update UI when state changes
  $: {
    step; error; message; progress;
    updateUIState();
  }
  // Initialize immediately with self-executing function
(async function initOnLoad() {
  try {
    console.log('Auto-initializing FlashManager on page load');
    ...
    // Initialize manager
    await qdlManager.initialize(imageManager);
    console.log('FlashManager auto-initialized successfully!');
...
<div id="flash" class="relative flex flex-col gap-8 justify-center items-center h-full">
  <div
    class="p-8 rounded-full {uiState.bgColor}"
    style="cursor: {(step === StepCode.READY && error === ErrorCode.NONE) ? 'pointer' : 'default'}"
    on:click={handleStart}
    on:keydown={(e) => e.key === 'Enter' && handleStart()}
    role="button"
    tabindex="0"
    aria-label="Start flashing"
  >
    <img
      src={uiState.icon}
      alt="cable"
      width="128"
      height="128"
      class="{uiState.iconStyle || 'invert'} {!error && step !== StepCode.DONE ? 'animate-pulse' : ''}"
    />
  </div>
  <div class="w-full max-w-3xl px-8 transition-opacity duration-300" style="opacity: {progress === -1 ? 0 : 1}">
    <div class="relative w-full h-2 bg-gray-200 rounded-full overflow-hidden">
      <div
        class="absolute top-0 bottom-0 left-0 w-full transition-all {uiState.bgColor}"
