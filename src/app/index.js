...
function createCopyText(text) {
  ...
  wrapper.className = 'relative text-sm'
  const pre = document.createElement('pre')
  pre.className = 'font-mono pt-12'
  pre.textContent = text
  const btn = document.createElement('button')
  btn.className = 'absolute top-2 right-2 bg-blue-600 hover:bg-blue-500 active:bg-blue-300 transition-colors text-white p-1 rounded-md'
...
class FlashApp extends HTMLElement {
  connectedCallback() {
    ...
    <div class="flex flex-col lg:flex-row flex-wrap">
      <main class="p-12 md:p-16 lg:p-20 xl:p-24 w-screen max-w-none lg:max-w-prose lg:w-auto h-auto lg:h-screen lg:overflow-y-auto prose dark:prose-invert prose-green bg-white dark:bg-gray-900">
        <section>
          ...
          <p>
            This tool allows you to flash AGNOS onto your comma device. AGNOS is the Ubuntu-based operating system for your <a href="https://comma.ai/shop/comma-3x" target="_blank">comma 3/3X</a>.
          </p>
        </section>
        <hr />
    ...
    if (isLinux) {
      const holder = this.querySelector('#detach-script')
      holder.replaceWith(createCopyText(DETACH_SCRIPT))
    }
  }
}

customElements.define('flash-app', FlashApp)
